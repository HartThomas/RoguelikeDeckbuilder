...
func _ready() -> void:
	...
	for i in range(total_cards):
		...
		card_instance.card_info = BattleManager.battleInfo.deck.filter(func(card) : return card.attack != 0 or card.block != 0)[i]
...
func remove_card_from_deck(card) -> void:
	finishing = true
	BattleManager.battleInfo.upgrade_card(card.card_info, 2)
	card_picked.emit()
