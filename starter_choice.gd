...
func _on_mouse_exited() -> void:
	if animation_player.current_animation == "hover" and animation_player.is_playing():
		# Play backwards from current position
		var time = animation_player.current_animation_position
		animation_player.play("hover", -1, -1.0, true)  # from_end = true
		animation_player.seek(time, true)
		animation_player.advance(0)  # Apply immediately
	else:
		# Fully reverse from end if not already playing
